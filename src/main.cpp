...
#if defined(NDEBUG)
# error "ZBKCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x34e6cc02535152f485d029266e3a25a126f5c4dc8a6bccd46d2ba2520a39fcf7"); //xreplace
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 5); // ZBKCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ZBKCoin Signed Message:\n";
...
{
    // ZBKCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 60 * 60; // ZBKCoin: 3.5 days
static const int64 nTargetSpacing =  60; // ZBKCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ZBKCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ZBKCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xffd0b796219c2451b8634c0bcf631d40f31f177639a12275dc5656bf734d1b3a"); //xreplace
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "A8 music is here 2018.5.14";//xreplace
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1523262263;//xreplace
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2859694; //xreplace

        if (fTestNet)
        {
            block.nTime    = 1523262263;
            block.nNonce   = 2522500;
        }

        //// debug print
        uint256 hash = block.GetHash();
        printf("orgig genesis block hash%s\n", hash.ToString().c_str());
        printf("set genesis block hash %s\n", hashGenesisBlock.ToString().c_str());
        printf("genesis merkleRoot hash %s\n", block.hashMerkleRoot.ToString().c_str());
        //   assert(block.hashMerkleRoot == uint256("0x923ebaa3b2c52c0d6214d9121e90dc4d42a12dd849acc6f8531dcdeacd29ce14"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // ZBKCoin: increase each by adding 2 to bitcoin's value. //�滻
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZBKCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZBKCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZBKCoinMiner(CWallet *pwallet)
{
    printf("ZBKCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ZBKCoin-miner");
    ...
    try { loop {
        ...
        printf("Running ZBKCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZBKCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZBKCoinMiner, pwallet));
}
